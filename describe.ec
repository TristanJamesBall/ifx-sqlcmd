...
@(#)Version:        $Revision: 2010.2 $
@(#)Last changed:   $Date: 2010/04/12 01:25:46 $
@(#)Purpose:        Allocate space for SQLDA structure
@(#)Author:         J Leffler
@(#)Copyright:      (C) JLSS 1992-93,1995-2001,2003-05,2007-08,2010
...
const char jlss_id_describe_ec[] = "@(#)$Id: describe.ec,v 2010.2 2010/04/12 01:25:46 jleffler Exp $";
#endif /* lint */
...
static int      jtypcsize(int sqltyp, int sqllen)
{
    ...
    case SQLUDTFIXED:
        /*
        ** JL 2008-07-13: Whatever its other merits, doing this broke
        ** LVARCHAR loading in SQLCMD - because instead of recording
        ** LVARCHAR as 2048 bytes, it was changed to 4 or 8 bytes.  Leave
        ** this alone for the time being, but something has to change,
        ** sooner or later.  One plausible design (OK, semi-plausible
        ** design), is to have sql_describe() allocate a second Sqlda,
        ** which is described in terms of the C structures, leaving the
        ** original in terms of the SQL types.  The code in SQLCMD that
        ** needs the original info can then find it; the code that tinkers
        ** with data can use the new structure.
        */
        rlen = sizeof(void *);
        break;
#endif /* ESQLC_IUSTYPES */
...
static size_t sql_descsize(Sqlda *desc)
{
    ...
    for (col = desc->sqlvar, i = 0; i < desc->sqld; col++, i++)
    {
        ...
        offset += size;
        TRACE((9, "A%d: type = %d, len = %d, size = %d\n", i, col->sqltype, col->sqllen, size));
    }
